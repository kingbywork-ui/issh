...
import { ConfigService, FileProvidersService, NotificationsService, PromptModalComponent, LogService, Logger, TranslateService, Platform, HostAppService } from 'issh-core'
    ...
    async start (): Promise<void> {
        ...
        this.ssh.x11ChannelOpen$.subscribe(async event => {
            ...
            try {
                ...
                this.emitServiceMessage(`    issh tried to connect to ${JSON.stringify(X11Socket.resolveDisplaySpec(displaySpec))} based on the DISPLAY environment var (${displaySpec})`)
        ...
        while (true) {
            ...
            if (method.type === 'publickey') {
                try {
                    ...
                    const hashAlgorithms: Array<'sha1'|'sha256'|'sha512'|null> = key.algorithm === 'ssh-rsa'
                        ? ['sha512', 'sha256', null]
                        : [null]

                    for (const hashAlgorithm of hashAlgorithms) {
                        const algorithmLabel = hashAlgorithm ?? 'default'
                        this.emitServiceMessage(`Trying private key: ${method.name} (${key.algorithm}, ${algorithmLabel})`)
                        const result = await this.ssh.authenticateWithKeyPair(this.authUsername, key, hashAlgorithm)
                        if (result instanceof russh.AuthenticatedSSHClient) {
                            return result
                        }
                        maybeSetRemainingMethods(result)
                        if (methodsLeft.length && !methodsLeft.includes('publickey')) {
                            break
                        }
                    }
                } catch (e) {
        ...
        while (true) {
            try {
                ...
                if (
                    ...
                    errorText.includes('IncorrectMac') ||
                    errorText.includes('Crypto') ||
                    errorText.includes('passphrase')
                ) {
