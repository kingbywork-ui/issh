...
import { PlatformService, LogService, Logger } from 'issh-core'
...
for path in paths:
    if path.exists():
        print(f'@@ISSH_HISTORY_FILE@@ {path}')
        try:
...
PY`,
    ...
    getTabKey (tab: BaseTerminalTabComponent<any>): string {
        ...
        if (!target.__isshLLMKey) {
            ...
            target.__isshLLMKey = `${profilePart}:${sessionPart}:${Date.now().toString(36)}:${Math.random().toString(36).slice(2, 8)}`
        }
        return target.__isshLLMKey
    }
