...
import { ConfigProxy, ProfileGroup, Profile, ProfileProvider, PlatformService, TranslateService, PartialProfileGroup, ProfilesService, TAB_COLORS } from 'issh-core'
...
export class EditProfileGroupModalComponent<G extends ProfileGroup> {
    ...
    getValidParents (groups: PartialProfileGroup<ProfileGroup>[], targetId: string): PartialProfileGroup<ProfileGroup>[] {
        ...
        return groups.filter(group => group.id !== targetId && !descendants.has(group.id))
    }
    ...
    colorsFormatter = value => TAB_COLORS.find(x => x.value === value)?.name ?? value
