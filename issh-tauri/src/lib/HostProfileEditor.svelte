<script lang="ts">
    ...
    let draft = $state({ ...profile, tags: [...profile.tags], privateKeys: [...profile.privateKeys], loginScript: profile.loginScript ?? '' })
    let tags = $state(draft.tags.join(', '))
    const isNew = $derived(!profile.id)

    function save (): void {
        onconnect({ ...draft, id: draft.id || `profile-${Date.now().toString(36)}`, name: draft.name.trim(), host: draft.host.trim(), user: draft.user.trim(), port: Number(draft.port) || 22, group: draft.group || '', tags: tags.split(',').map((tag) => tag.trim()).filter(Boolean), loginScript: draft.loginScript.trim() || null })
    }
</script>
