...
before(async () => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tabby-agent-test-'))
})
...
test('loads a validated connection and calls loopback RPC', async () => {
    ...
    const result = await rpc(connection, 'tabby_health', { check: true })
    assert.deepEqual(result, { method: 'tabby_health', params: { check: true } })
})
