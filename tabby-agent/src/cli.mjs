...
export function usage () {
  ...
  tabby-agent output --output-id <id> [--offset 0] [--limit 8000]
  tabby-agent batch-exec [--tabs all-ssh|tab-1,tab-2] [--serial] [--timeout-ms 60000] -- <command>
...
export function buildCall (command, options, positionals) {
    ...
    const calls = {
        ...
        connect: ['tabby_connect_profile', compact({ id: options.id, name: options.name, timeoutMs: options.timeoutMs })],
        ...
        preview: ['tabby_preview_command', { ...baseParams, command: content }],
        insert: ['tabby_insert_command', { ...baseParams, command: content }],
        run: ['tabby_run_command', { ...baseParams, command: content, confirmDangerous: options.confirmDangerous }],
        exec: ['tabby_exec_command', compact({ ...baseParams, command: content, timeoutMs: options.timeoutMs, cwd: options.cwd, confirmDangerous: options.confirmDangerous })],
        output: ['tabby_get_output', { outputId: options.outputId, offset: options.offset, limit: options.limit }],
        'batch-exec': ['tabby_batch_exec', compact({ tabs: tabs.length === 1 ? tabs[0] : tabs, command: content, timeoutMs: options.timeoutMs, cwd: options.cwd, parallel: options.parallel, confirmDangerous: options.confirmDangerous })],
        ...
        'sftp-write': ['tabby_sftp_write', { ...baseParams, path: options.path, encoding: options.encoding, content }],
    }
