...
const tabProperty = {
    type: 'string',
    description: 'Tab id returned by tabby_list_sessions, or "active".',
}
...
const confirmDangerousProperty = {
    type: 'boolean',
    description: 'Signals that the agent expects a dangerous-command confirmation. Tabby still requires native user approval.',
}
...
export const AGENT_BRIDGE_TOOLS = [
    {
        name: 'tabby_health',
        scope: 'read',
        description: 'Check whether the Tabby Agent Bridge is reachable and report its basic state.',
        inputSchema: {
            type: 'object',
            properties: {},
    ...
    },
    {
        ...
        scope: 'read',
        description: 'List terminal sessions currently registered in Tabby, including stable tab ids and connection metadata.',
        inputSchema: {
            type: 'object',
            properties: {},
            additionalProperties: false,
    ...
    },
    {
        ...
        scope: 'write',
        description: 'Close a Tabby terminal session.',
        inputSchema: {
            type: 'object',
    ...
    },
    {
        ...
        scope: 'write',
        description: 'Select a Tabby terminal session and make it active in the application UI.',
        inputSchema: {
            type: 'object',
            properties: { tab: tabProperty },
            additionalProperties: false,
    ...
    },
    {
        ...
        scope: 'read',
        description: 'Normalize a command and report whether Tabby considers it dangerous without executing it.',
        inputSchema: {
            type: 'object',
            required: ['command'],
            properties: {
    ...
    },
    {
        ...
        scope: 'exec',
        description: 'Execute a command and wait for isolated output and an exit code. SSH sessions use an exec channel; local sessions use a separate matching shell process. Use tabby_run_command when the command must run visibly inside the interactive terminal.',
        inputSchema: {
            type: 'object',
            required: ['command'],
            properties: {
    ...
    {
        name: 'tabby_get_output',
        scope: 'read',
        description: 'Read another page of output returned by a truncated tabby_exec_command result.',
        inputSchema: {
            type: 'object',
            required: ['outputId'],
            properties: {
                outputId: { type: 'string', minLength: 1, description: 'Output id returned by tabby_exec_command.' },
...
export const AGENT_BRIDGE_METHOD_SCOPES = Object.freeze(Object.fromEntries(
