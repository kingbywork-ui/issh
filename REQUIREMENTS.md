...
- 内置 Agent Bridge `59688` 继续只提供 MCP/CLI/RPC；商城 Agent Bridge 改为只读状态卡与 Web 入口。
- Tauri 管理服务器自动监听 `127.0.0.1:33555`，Dashboard 内嵌，令牌首次生成并持久化，显式轮换。
...
### R-095 Agent Hub 设置入口与管理凭据可见性修复（用户需求，2026-09-08，已完成）

- 现象：商城记录显示 Agent Hub Connector v0.4.0 已安装，但左侧没有 Agent Hub 设置菜单；用户看不到 Hub 地址和管理 token。
